...
export const Peer: React.FC<{ peer: HMSPeer }> = ({ peer }) => {
  ...
  return (
    <div 
      ...
      className={`peerTile m-2 relative rounded-lg shadow-md border-0 dark:border-4 transition duration-500 overflow-hidden flex justify-center items-center ${audioLevel > 0 ? " border-v9-pink" : " dark:border-white "}`} >
        <Video videoRef={videoRef} peer={peer} />
        <div className={`absolute bg-opacity-60 h-full w-full backdrop-filter backdrop-blur-sm flex justify-center items-center z-10 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${isVideoOn ? "invisible" : "visible"}`}>
            <Avatar
                size={200}
                name={peer.name}
                variant="ring"
        ...
        <div style={{maxWidth: "90px"}} className={`absolute z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center overflow-hidden text-ellipsis ${isVideoOn ? "-top-1 translate-y-0 bg-gray-100 px-2 text-black rounded-b-lg font-normal text-md py-0.5" : "text-white font-medium text-xl"}`}>{peer.name}</div>
