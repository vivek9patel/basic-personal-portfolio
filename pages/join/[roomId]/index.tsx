...
const JoinRoomForm: () => JSX.Element = () => {
  ...
  const roleName: string = appState.state.isOwner
    ? process.env.NEXT_PUBLIC_ADMIN_ROLE || "guest"
    : "guest";
  ...
  return (
    ...
    >
      <div className="rounded-lg flex-1 mx-6 overflow-hidden shadow-lg bg-white border-2 dark:border-gray-300 dark:bg-transparent dark:text-white text-gray-900 max-w-sm">
        <div className=" w-full h-80 md:h-96 sm:w-96 relative flex justify-center items-center">
          {localPeer && <Video videoRef={videoRef} peer={localPeer} />}
          <div
            className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${
              isLocalVideoEnabled ? "invisible" : "visible"
            }`}
          >
            <Avatar
              size={200}
        ...
        <div className="p-6">
          ...
          <div className="flex mt-4 justify-evenly items-center w-full dark:text-black">
            <button
              className={`${
                isLocalAudioEnabled ? "" : "bg-v9-pink"
              } w-10 h-10 flex items-center justify-center rounded-full shadow-lg transition duration-300 ease-in-out dark:text-white`}
              type="button"
              onClick={toggleAudio}
            >
              {isLocalAudioEnabled ? (
            ...
            <button
              className={`${
                isLocalVideoEnabled ? "" : "bg-v9-pink"
              } w-10 h-10 flex items-center justify-center rounded-full shadow-lg transition duration-300 ease-in-out dark:text-white`}
              type="button"
              onClick={toggleVideo}
            >
